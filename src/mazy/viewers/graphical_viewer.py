...
class MazeGraphicalProcessor:
    ...
    def calculate_cell_points(self, cell: Cell) -> list[Point]:
        ...
        if not cell.role == Role.ENTRANCE and cell.row == 0:
            points.append(Point(start_x, self.delta_y))
            points.append(Point(end_x, self.delta_y))

        # First vertical line (frontier on WEST)
        if cell.col == 0:
            points.append(Point(EXTERNAL_SIZE, start_y))
            points.append(Point(EXTERNAL_SIZE, end_y))
        ...
        if not cell.role == Role.EXIT and cell.row == self.rows - 1:
            points.append(Point(start_x, EXTERNAL_SIZE))
            points.append(Point(end_x, EXTERNAL_SIZE))
    ...
    def process_maze(self) -> list[Point]:
        ...
        for cell in self.maze.traverse_by_cell():
            cell_points.extend(self.calculate_cell_points(cell))
...
class MazeGraphicalRenderer(Window):
    ...
    def on_update(self, delta_time: float) -> None:
        ...
        line_points = self.processor.process_maze()
        maze_shapes: ShapeElementList[Shape | Any] = ShapeElementList()  # type: ignore[no-untyped-call]
        maze_shapes.append(create_lines(point_list=line_points, color=BORDER_COLOR))

    def on_draw(self) -> None:
        """Render all objects for the active window."""
        self.clear()
        self.maze_shapes.draw()
